...
labeling_function = labeling_function = labeling_function = lambda row: 1 if row["rating"] > 3 else 0  # TODO: Define your labeling function here. # TODO: Define your labeling function here.
...
class BaselineChallenge_01(FlowSpec):
    ...
    def baseline(self):
        ...
        majority_clf.fit(self.traindf["review"], self.traindf["label"])
    
        predictions = majority_clf.predict(self.valdf["review"]) # TODO: predict the majority class
    ...
    def model(self):
        ...
        from model_01 import NbowModel
        ...
        self.hyperparam_set = [{'vocab_sz': 100, "epochs": 20, "batch_size": 32}, {'vocab_sz': 300, "epochs": 20, "batch_size": 32}, {'vocab_sz': 500, "epochs": 20, "batch_size": 32}]  
        pathspec = f"{current.flow_name}/{current.run_id}/{current.step_name}/{current.task_id}"

        self.results = []
        for params in self.hyperparam_set:
            model = NbowModel(params["vocab_sz"], params["epochs"], params["batch_size"]) # TODO: instantiate your custom model here!
